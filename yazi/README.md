...
Yazi uses flavors for UI theming. The active flavor is hardcoded to
`catppuccin-mocha` in `theme.toml`, matching the rest of the setup:
...
The `catppuccin-mocha.yazi` flavor ships in `flavors/`. To switch, drop another
flavor into `flavors/` and point `theme.toml` at it.
